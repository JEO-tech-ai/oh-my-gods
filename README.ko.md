...
> **PLAN 피드백 루프 가드**: "Send Feedback" 이후 plan.md 내용이 반드시 변경되어야 재진입 가능합니다. 내용이 동일하면(같은 hash) `⚠️ PLAN_LOOP` 경고와 함께 재진입이 차단됩니다 — 수정 없이 EXECUTE로 넘어가는 오작동을 방지합니다.
...
| `plannotator` | `plan` | AI 생성 계획 브라우저 리뷰 UI; 승인 또는 피드백 전송. **피드백 루프 가드**: 피드백 후 plan.md가 수정되지 않으면 재진입 차단 (오승인 방지). | [plannotator](https://plannotator.ai) |
